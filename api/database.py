...
class DescriptionToken(Base):
    """Normalized word tokens extracted from feature descriptions with frequency counts."""

    __tablename__ = "description_tokens"
...
def _migration_v7(engine) -> None:
    """v7: Create description_tokens table for storing normalized word tokens from feature descriptions."""
    from sqlalchemy import text

    with engine.connect() as conn:
        result = conn.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='description_tokens'"))
        if result.fetchone() is None:
            conn.execute(text("""
                CREATE TABLE description_tokens (
                    token TEXT PRIMARY KEY,
                    usage_count INTEGER NOT NULL DEFAULT 0
                )
            """))
