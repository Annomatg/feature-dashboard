...
class TestGetAgentLog:
    """Unit tests for GET /api/tasks/{id}/agent/{agent_id}/log."""
    ...
    def test_task_not_found_returns_404(self, client):
        """Returns 404 when task ID doesn't exist."""
        response = client.get("/api/tasks/999/agent/main/log")
        assert response.status_code == 404
        assert "not found" in response.json()["detail"].lower()
    ...
    def test_main_agent_success(self, client, monkeypatch, tmp_path):
        """Returns structured turns for the main agent."""
        ...
        assert "Fix the bug" in data["turns"][0]["content"]
        assert data["turns"][1]["role"] == "assistant"
        assert "I will fix it" in data["turns"][1]["content"]

    def test_main_agent_session_file_not_found_returns_404(self, client, monkeypatch):
        """Returns 404 when main session file doesn't exist."""
        self._set_session_id(client, "missing.jsonl")
    ...
    def test_subagent_success(self, client, monkeypatch, tmp_path):
        """Returns structured turns for a subagent."""
        import json as json_lib
        self._set_session_id(client, "test-session.jsonl")
        ...
        response = client.get("/api/tasks/1/agent/abc123/log")

        assert response.status_code == 200
        data = response.json()
        assert len(data["turns"]) == 1
        assert data["turns"][0]["role"] == "user"
        assert "Subagent task" in data["turns"][0]["content"]

    def test_subagent_not_found_returns_404(self, client, monkeypatch):
        """Returns 404 when agent_id is not in subagents list."""
        self._set_session_id(client, "test-session.jsonl")
        ...
        response = client.get("/api/tasks/1/agent/nonexistent/log")
        assert response.status_code == 404
        assert "nonexistent" in response.json()["detail"].lower()

    def test_subagent_file_missing_on_disk_returns_404(self, client, monkeypatch, tmp_path):
        """Returns 404 when subagent log file doesn't exist on disk."""
        self._set_session_id(client, "test-session.jsonl")
    ...
    def test_limit_parameter_clamped(self, client, monkeypatch, tmp_path):
        ...
        response = client.get("/api/tasks/1/agent/main/log?limit=999")
        assert response.status_code == 200

    def test_returns_role_and_content_fields(self, client, monkeypatch, tmp_path):
        """Each turn has role, content, and timestamp fields."""
        import json as json_lib
        self._set_session_id(client, "test-session.jsonl")
        ...
        assert turn["timestamp"] == "2026-03-11T08:00:00Z"
