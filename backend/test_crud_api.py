...
# ==============================================================================
# Comment endpoint tests (routers/comments.py)
# ==============================================================================

class TestCommentEndpoints:
    """Tests for GET/POST/DELETE /api/features/{id}/comments (Feature #164)."""
    ...
    def test_add_comment_success(self, client):
        """POST creates a comment and returns 201 with comment data."""
    ...
    def test_add_comment_feature_not_found(self, client):
        """POST returns 404 when feature does not exist."""
        response = client.post("/api/features/999/comments", json={"content": "Hello"})
        assert response.status_code == 404
        assert "not found" in response.json()["detail"].lower()

    def test_add_comment_empty_content(self, client):
        """POST returns 400 when content is empty or whitespace-only."""
        response = client.post("/api/features/1/comments", json={"content": "  "})
        assert response.status_code == 400
        assert "empty" in response.json()["detail"].lower()

    def test_add_comment_strips_whitespace(self, client):
        """POST trims leading/trailing whitespace from content."""
        response = client.post("/api/features/1/comments", json={"content": "  trimmed  "})
        assert response.status_code == 201
        assert response.json()["content"] == "trimmed"

    def test_get_comments_returns_all_in_order(self, client):
        """GET returns all comments ordered by created_at ascending."""
    ...
    def test_delete_comment_success(self, client):
        """DELETE removes a comment and returns 204."""
        ...
        # Verify it's gone
        get = client.get("/api/features/1/comments")
        assert all(c["id"] != comment_id for c in get.json())

    def test_delete_comment_not_found(self, client):
        """DELETE returns 404 when comment does not exist."""
        response = client.delete("/api/features/1/comments/9999")
        assert response.status_code == 404
        assert "not found" in response.json()["detail"].lower()

    def test_delete_comment_wrong_feature(self, client):
        """DELETE returns 404 when comment belongs to a different feature."""
        post = client.post("/api/features/1/comments", json={"content": "Feature 1 comment"})
        assert post.status_code == 201
        comment_id = post.json()["id"]

        # Try to delete via feature 2 — should fail
        response = client.delete(f"/api/features/2/comments/{comment_id}")
        assert response.status_code == 404

    def test_comments_isolated_per_feature(self, client):
        """Comments for one feature do not appear in another feature's list."""
        client.post("/api/features/1/comments", json={"content": "Feature 1 note"})
        client.post("/api/features/2/comments", json={"content": "Feature 2 note"})

        r1 = client.get("/api/features/1/comments")
        r2 = client.get("/api/features/2/comments")

        assert all(c["feature_id"] == 1 for c in r1.json())
        assert all(c["feature_id"] == 2 for c in r2.json())
